...
import { Wallet, CheckCircle, AlertCircle, TrendingUp, TrendingDown, Plus, Banknote, Calendar, Receipt, History, ChevronLeft, ChevronRight, Edit2, DollarSign, Tag, Layers, BarChart3, PieChart, ArrowUpCircle, ArrowDownCircle } from 'lucide-react';
...
const Finance: React.FC<Props> = ({ payments, players, setPayments, expenses, setExpenses }) => {
  ...
  const totalRevenue = useMemo(() => payments.filter(p => p.status === 'Pago').reduce((acc, p) => acc + (Number(p.value) || 0), 0), [payments]);
  const totalExpenses = useMemo(() => expenses.reduce((acc, e) => acc + (Number(e.value) || 0), 0), [expenses]);
  const globalBalance = totalRevenue - totalExpenses;
  
  // Period Specific Logic (Analysis)
  ...
  const periodRevenueItems = useMemo(() => {
    return payments.filter(p => p.month === selectedPeriod && p.status === 'Pago');
  ...
  // Filtering Payments for Progress
  const periodPayments = useMemo(() => payments.filter(p => p.month === selectedPeriod), [payments, selectedPeriod]);
  const paidInPeriodCount = periodPayments.filter(p => p.status === 'Pago').length;
  const progress = players.length > 0 ? (paidInPeriodCount / players.length) * 100 : 0;

  const togglePayment = (playerId: string) => {
    setPayments(prev => {
      const existing = prev.find(p => p.playerId === playerId && p.month === selectedPeriod);
      if (existing) {
        return prev.map(p => (p.playerId === playerId && p.month === selectedPeriod) 
  ...
  const updatePaymentValue = (playerId: string, newValue: string) => {
    ...
    setPayments(prev => {
      const existing = prev.find(p => p.playerId === playerId && p.month === selectedPeriod);
      if (existing) {
        return prev.map(p => (p.playerId === playerId && p.month === selectedPeriod) ? { ...p, value: val } : p);
      } else {
  ...
  const historyItems = useMemo(() => {
    const items = [
      ...expenses.map(e => ({ ...e, type: 'expense' as const, sortDate: new Date(e.date).getTime() })),
      ...payments.filter(p => p.status === 'Pago').map(p => {
        const player = players.find(pl => pl.id === p.playerId);
        return {
          id: `${p.playerId}-${p.month}`,
          description: `Mensalidade: ${player?.name || 'Atleta'} (${p.month})`,
          value: p.value,
          date: p.month,
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Main Balance Card - Global Context */}
      {activeTab !== 'analysis' && (
      ...
      <div className="bg-[#0A0A0A] p-4 rounded-3xl border border-white/5 space-y-4">
        ...
        <div className="flex gap-2">
          <select 
            className="flex-1 bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-xs font-bold text-white outline-none focus:border-[#F4BE02]/50 transition-all active:scale-[0.98]"
            value={selectedMonth}
          ...
          <select 
            className="w-24 bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-xs font-bold text-white outline-none focus:border-[#F4BE02]/50 transition-all active:scale-[0.98]"
            value={selectedYear}
            onChange={(e) => setSelectedYear(e.target.value)}
          >
            {[currentYear-1, currentYear, currentYear+1].map(y => <option key={y} value={y} className="bg-black">{y}</option>)}
          </select>
        </div>
      </div>
          ...
          <section className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div className="px-1 flex justify-between items-end">
              <div>
                  ...
                  <p className="text-[9px] text-white/20 mt-1">{selectedPeriod}</p>
              </div>
            ...
            <div className="space-y-3 pb-32">
              {players.map(player => {
                const payment = payments.find(p => p.playerId === player.id && p.month === selectedPeriod);
                  ...
                  <div key={player.id} className="bg-[#0A0A0A] p-4 rounded-[24px] border border-white/[0.06] flex flex-col gap-4">
                    <div className="flex items-center justify-between">
                      ...
                      <button 
                        ...
                        className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${isPaid ? 'bg-green-500 text-white' : 'bg-white/5 text-white/40 border border-white/5'}`}
                      >
                  ...
                  <div className="bg-[#0A0A0A] p-5 rounded-[24px] border border-white/10 space-y-4 animate-in slide-in-from-top-4">
                      ...
                      <div className="space-y-4">
                          <div className="bg-white/5 rounded-xl p-3 flex items-center gap-3 border border-white/5 focus-within:border-[#F4BE02]/50 transition-colors">
                              ...
                              <input type="text" placeholder="Descrição" className="bg-transparent w-full outline-none text-sm font-bold placeholder:text-white/20" value={newExpenseDesc} onChange={e => setNewExpenseDesc(e.target.value)} />
                          </div>
                          ...
                          <button onClick={handleAddExpense} disabled={!newExpenseDesc || !newExpenseValue} className="w-full bg-red-600 text-white py-3 rounded-xl font-black text-xs uppercase tracking-widest shadow-lg shadow-red-500/20 disabled:opacity-50 transition-all active:scale-[0.98]">Confirmar Saída</button>
                      </div>
                  </div>
              )}
              <div className="space-y-3">
                  <div className="flex items-center justify-between px-1">
                    <h3 className="text-xs font-black uppercase tracking-widest text-white/40">Saídas Filtradas: {selectedPeriod}</h3>
                    <p className="text-[8px] font-black text-white/20 uppercase tracking-widest">Clique no valor para ajustar</p>
                  </div>
                  {filteredExpenses.length > 0 ? filteredExpenses.map(expense => (
                      <div key={expense.id} className="bg-[#0A0A0A] p-4 rounded-[24px] border border-white/[0.06] flex items-center justify-between group transition-all hover:border-white/10">
                          <div className="flex items-center gap-4">
                              ...
                              <div>
                                  <div className="flex items-center gap-2">
                                    <h4 className="font-bold text-sm text-white truncate max-w-[120px]">{expense.description}</h4>
                                    <span className={`text-[7px] font-black uppercase px-1.5 py-0.5 rounded ${expense.category === 'Fixa' ? 'bg-[#F4BE02]/10 text-[#F4BE02]' : 'bg-blue-500/10 text-blue-500'}`}>
                                      {expense.category || 'Variável'}
                                    </span>
                                  </div>
                          ...
                          <div className="flex items-center gap-1 bg-white/[0.02] px-2 py-1 rounded-lg border border-white/5 focus-within:border-red-500/30">
                            <span className="text-[10px] font-bold text-red-500/50">R$</span>
                            <input 
                              type="number"
                              value={expense.value}
                      ...
                      <div className="text-center py-10 opacity-20"><Receipt size={40} className="mx-auto mb-3" /><p className="text-[10px] font-black uppercase tracking-widest">Nenhuma despesa para este período</p></div>
                  )}
              </div>
          </section>
      )}
      ...
      {activeTab === 'analysis' && (
          <section className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300 pb-32">
            <div className="bg-white/[0.03] border border-white/5 rounded-[32px] p-6 space-y-6">
                <div className="flex items-center justify-between">
                    <div>
                        <h3 className="text-lg font-display font-bold">Resumo Mensal</h3>
                        <p className="text-[10px] font-black text-[#F4BE02] uppercase tracking-widest">{selectedPeriod}</p>
                    </div>
                ...
                <div className="grid grid-cols-1 gap-3">
                    ...
                    <div className="flex items-center justify-between px-2">
                        ...
                        <span className={`font-display font-bold text-xl ${periodBalance >= 0 ? 'text-white' : 'text-red-500'}`}>
                            {periodBalance < 0 ? '-' : '+'} R$ {Math.abs(periodBalance).toFixed(2)}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-3">
                <div className="flex items-center gap-2 px-1">
                    ...
                    <h3 className="text-xs font-black uppercase tracking-widest text-white/40">Detalhamento de Receitas</h3>
                </div>
                        ...
                        return (
                            <div key={item.playerId} className="p-4 flex items-center justify-between">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-xs font-bold text-white/80">{player?.name || 'Atleta'}</span>
                                </div>
                                <span className="text-sm font-display font-bold text-green-500">R$ {item.value.toFixed(2)}</span>
                            </div>
                        );
                    }) : (
                        <div className="p-10 text-center opacity-20 text-[10px] font-black uppercase tracking-widest">Nenhuma receita confirmada</div>
                    )}
                </div>
            </div>
            ...
            <div className="space-y-3">
                <div className="flex items-center gap-2 px-1">
                    ...
                    <h3 className="text-xs font-black uppercase tracking-widest text-white/40">Detalhamento de Despesas</h3>
                </div>
                <div className="bg-[#0A0A0A] rounded-[24px] border border-white/[0.06] divide-y divide-white/[0.03]">
                    {filteredExpenses.length > 0 ? filteredExpenses.map(expense => (
                        <div key={expense.id} className="p-4 flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <p className="text-xs font-bold text-white/80">{expense.description}</p>
                                    <p className="text-[8px] font-black text-white/20 uppercase tracking-widest">{expense.category}</p>
                                </div>
                            </div>
                            <span className="text-sm font-display font-bold text-red-500">R$ {expense.value.toFixed(2)}</span>
                        </div>
                    )) : (
                        <div className="p-10 text-center opacity-20 text-[10px] font-black uppercase tracking-widest">Nenhuma despesa lançada</div>
                    )}
                </div>
            </div>
          </section>
      ...
      {activeTab === 'history' && (
        <section className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300 pb-32">
          <div className="px-1">
            ...
            <p className="text-[9px] text-white/20 mt-1">Todas as transações do clube</p>
          </div>
          <div className="space-y-3">
            {historyItems.length > 0 ? historyItems.map((item, idx) => (
              <div key={item.id + idx} className="bg-[#0A0A0A] p-4 rounded-[24px] border border-white/[0.06] flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div className="max-w-[180px]">
                    <div className="flex items-center gap-2">
                      <h4 className="font-bold text-sm text-white truncate">{item.description}</h4>
                      {item.type === 'expense' && (
                        <span className={`text-[6px] font-black uppercase px-1 rounded ${item.category === 'Fixa' ? 'bg-[#F4BE02]/10 text-[#F4BE02]' : 'bg-blue-500/10 text-blue-500'}`}>
                          {item.category}
                        </span>
                      )}
                    </div>
                    <span className="text-[9px] text-white/30 uppercase font-black tracking-widest">{item.date}</span>
                  </div>
                </div>
              ...
              <div className="text-center py-20 opacity-20"><History size={48} className="mx-auto mb-4" /><p className="text-[10px] font-black uppercase tracking-widest">Histórico Vazio</p></div>
            )}
          </div>
        </section>
      )}
...
const TabButton = ({ active, onClick, icon, label }: any) => (
    <button
      onClick={onClick}
      className={`py-3 rounded-xl text-[9px] font-black uppercase tracking-[0.15em] transition-all flex items-center justify-center gap-1.5 ${
         active ? 'bg-[#F4BE02] text-black shadow-lg' : 'text-white/30 hover:bg-white/5'
      }`}
    >
      {icon} {label}
    </button>
...
const AnalysisSummaryItem = ({ icon, label, value, color }: any) => (
    <div className="flex items-center justify-between px-2">
        ...
        <span className={`font-display font-bold text-sm ${color}`}>R$ {value.toFixed(2)}</span>
    </div>
);
